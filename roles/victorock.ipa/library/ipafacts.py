...
def main():
    ...
    if HAS_IPALIB:
        if is_client_configured():
            ipafacts['configured']['client'] = True

            ipafacts['version'] = get_ipa_version()
            for key,value in six.iteritems(get_ipa_conf()):
                ipafacts[key] = value

    if HAS_IPASERVER:
        if is_server_configured():
            ipafacts['configured']['server'] = True
            ipafacts['configured']['dns'] = is_dns_configured()
            ipafacts['configured']['ca'] = is_ca_configured()
            ipafacts['configured']['kra'] = is_kra_configured()
            ipafacts['configured']['ntpd'] = is_ntpd_configured()

    module.exit_json(
        changed=False,
        ansible_facts=dict(ipa=ipafacts)
        )
